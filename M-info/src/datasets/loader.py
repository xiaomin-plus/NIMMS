...
def move_to_cuda(batch, device_id):
    if isinstance(batch, torch.Tensor):
        # print("batch: tensor: ",batch[0])
        return batch.to(device = device_id)
    elif isinstance(batch, list):
        new_batch = [move_to_cuda(t, device_id) for t in batch]
    elif isinstance(batch, tuple):
        new_batch = tuple(move_to_cuda(t, device_id) for t in batch)
    elif isinstance(batch, dict):
        new_batch = {n: move_to_cuda(t, device_id) for n, t in batch.items()}
    else:
        return batch
    return new_batch
...
class PrefetchLoader(object):
    ...
    """
    def __init__(self, loader, device_id):
        self.loader = loader
    ...
    def preload(self, it):
        ...
        with torch.cuda.stream(self.stream):
            self.batch = move_to_cuda(self.batch, self.device_id)
    ...
    def next(self, it):
        # print("loader.py 134: ",self.stream)
