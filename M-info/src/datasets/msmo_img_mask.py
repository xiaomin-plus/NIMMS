...
class REFER(Dataset):
	...
	def __getitem__(self, index):
		...
		mask_region_id = 0
		for i, img_n in enumerate(img_list):
			...
			if i < region_index[0]:
				if i == 0:
					mask_region_id = mask_region_id + num_bb
				else:
					mask_region_id = mask_region_id + cur_num_bb
			elif i == region_index[0]:
				mask_region_id = mask_region_id + region_index[1]
...
def batch_extract_mask_word_all(cand_num = 3, truncate_img = 20):
	import glob
	data_subs = ["test_data","data1"]
	crop_dir = "/data-yifan/data/meihuan2/dataset/MSMO_Finished/test_data/crop_caption"
	img_feat_dir = "/data-yifan/data/meihuan2/dataset/MSMO_Finished/test_data/img_roi"
	ex_path = "/data-yifan/data/meihuan2/dataset/MSMO_Finished/exID_bert_test.pickle"
