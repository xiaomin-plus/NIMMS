...
sys.path.append('../')
from src.model.TextEncoder_drop import UniterForVisualSummEnc
...
class LossClassifier(nn.Module):
    def __init__(self, config, hidden_dim, output_class):
        super(LossClassifier, self).__init__()
        self.dense =  nn.Linear(hidden_dim,output_class)
        self.activation = nn.Tanh()
        if config.loss_nums == 3:
            self.pad = False
        else:
            self.pad = True
...
class LossScorer(nn.Module):
    def __init__(self, config, hidden_dim):
        super(LossScorer, self).__init__()
        self.config = config
        self.dense_eg =  nn.Linear(hidden_dim,1)
        self.dense_vg =  nn.Linear(hidden_dim,1)
        self.dense_mg =  nn.Linear(hidden_dim,1)
        if config.loss_nums == 4:
            self.dense_none = nn.Linear(hidden_dim * 3, 1)
            self.pad = True
        else:
            self.pad = False
        self.activation = nn.Tanh()
    ...
    def forward(self, hidden_state,txt_enc, img_enc):
        eg_score = self.dense_eg(img_enc)
        eg_score = self.activation(eg_score)

        vg_score = self.dense_vg(txt_enc)
        vg_score = self.activation(vg_score)

        mg_score = self.dense_mg(hidden_state)
        mg_score = self.activation(mg_score)

        if self.config.loss_nums == 4:
            none_cat = torch.cat((img_enc, txt_enc, hidden_state), dim=1)
            none_score = self.dense_none(none_cat)
            none_score = self.activation(none_score)
            scores = torch.cat((eg_score, vg_score, mg_score, none_score), dim=1)
        else:
            scores = torch.cat((eg_score, vg_score, mg_score), dim=1)
...
class LossGlobalPart(nn.Module):
    def __init__(self, config, hidden_dim):
        ...
        self.dense_vg = nn.Linear(hidden_dim, 1)
        self.dense_mg = nn.Linear(hidden_dim, 1)
        if self.config.loss_nums == 3:
            self.dense_none = nn.Linear(hidden_dim * 2, 1)
            self.pad = True
        elif self.config.loss_nums == 2:
            self.pad = False
        self.activation = nn.Tanh()
    ...
    def forward(self, hidden_state, txt_enc, img_enc):
        ...
        if self.config.loss_nums == 3:
            ...
            scores = torch.cat((vg_score, mg_score, none_score), dim=1)
        else:
...
class MultiModal(nn.Module):
    def __init__(self, config, tokenizer, model_file_path=None, is_eval=False):
        ...
        if config.losskind == 'LossScorer':
            self.loss_scorer = LossScorer(config, hidden_dim)
        elif config.losskind == 'LossClassifier':
            self.loss_scorer = LossClassifier(config, hidden_dim, self.config.loss_nums)
        elif config.losskind == 'LossGlobalPart':
    ...
    def gen_attn(self, batch):
        ...
        for b in range(batch_size):
            ...
            attn_vis_RU[b, :txt_lens[b], txt_lens[b]:] = 1
            attn_vis_LD[b, txt_lens[b]:, :txt_lens[b]] = 1

        attn_vis_self = attn_vis_self * _attn_eys
        return attn_masks, attn_vis_RU, attn_vis_LD, attn_vis_self, attn_vis_all,attn_txt_all
    ...
    def forward(self, batch, encoder_training=False):
        ...
        attn_masks, attn_vis_RU, attn_vis_LD, attn_vis_self, attn_vis_all,attn_txt_all = self.gen_attn(batch)
        batch['attn_masks'] = attn_masks
        batch['attn_vis_RU'] \
            = attn_vis_RU
        batch['attn_vis_LD'] = attn_vis_LD
        batch['attn_vis_cross'] = attn_vis_RU + attn_vis_LD
        batch['attn_vis_self'] = attn_vis_self
        batch['attn_vis_all'] = attn_vis_all.unsqueeze(2)
        batch['attn_txt_all'] = attn_txt_all.unsqueeze(2)
        encoder_output, pooled_output, embedding_output, attn_masks, all_attention_scores,visual_outputs = self.text_encoder(batch, training=encoder_training)
        ...
        if self.config.losskind == 'LossScorer':
            loss_weight = self.loss_scorer(pooled_output, txt_enc, img_enc)
        elif self.config.losskind == 'LossClassifier':
            loss_weight = self.loss_scorer(pooled_output)
        elif self.config.losskind == 'LossGlobalPart':
            loss_weight = self.loss_scorer(pooled_output, txt_enc, img_enc)
        else:
            loss_weight = None

        return torch.stack(final_dists, dim=1), attn_dists, coverage, Ias,sim_loss, all_attention_scores, ot_dist, loss_weight,visual_outputs
    ...
    def decode(self, batch):
        ...
        encoder_output, pooled_output, embedding_output, attn_masks, all_attention_scores,visual_outputs = self.text_encoder(batch)
        max_dec_len = max(batch['dec_len'])
        b, t_k, n = list(encoder_output.size())
        s_t_1 = pooled_output.unsqueeze(0)
        if self.config.decoder_init and self.config.decoder_init_methods == 'global':
        ...
        if self.config.score_ref:
            ...
            sim_loss = None
        ot_dist = self.get_ot_dist(batch, encoder_output)
        return torch.stack(final_dists, dim=1), attn_dists, coverage, Ias,sim_loss, all_attention_scores, ot_dist,visual_outputs

    def sort_beams(self, beams):
        return sorted(beams, key=lambda h: h.avg_log_prob, reverse=True)

    def get_attn_diff(self, config, all_attention_scores,visual_outputs, attn_vis_RU, attn_vis_LD):
        visual_outputs = [visual_outputs[6]]
        ...
        diff_attention_scores = []
        b, size_a1, _ = visual_outputs[0].shape
        for attn, vis in zip(all_attention_scores, visual_outputs):
            #attn*vis[:,:,1:]-attn*vis[:,:,:1]
            nonvisual_attn = attn*vis[:,:,1:].expand(b,size_a1,size_a1)*attn_vis_RU+\
                             attn*vis[:,:,1:].transpose(1,2).expand(b,size_a1,size_a1)*attn_vis_LD
            visual_attn = attn*vis[:,:,:1].expand(b,size_a1,size_a1)*attn_vis_RU+\
                             attn*vis[:,:,:1].transpose(1,2).expand(b,size_a1,size_a1)*attn_vis_LD
            diff_attention_scores.append(nonvisual_attn-visual_attn)
        avg_attn_scores = sum(diff_attention_scores) / len(all_attention_scores)
