...
sys.path.append('../')
from src.model.TextEncoder_modulate import UniterForVisualSummEnc
...
class MultiModal(nn.Module):
    ...
    def gen_attn(self, batch):
        ...
        attn_vis_pos = torch.zeros((attn_masks_s0, attn_masks_s1, attn_masks_s1), dtype=torch.long)
        attn_vis_pos = attn_vis_pos.to(device=attn_masks.device)
        attn_vis_neg = torch.zeros((attn_masks_s0, attn_masks_s1, attn_masks_s1), dtype=torch.long)
        ...
        return attn_masks, attn_vis_pos, attn_vis_neg, attn_vis_self
    ...
    def forward(self, batch, encoder_training=False):
        ...
        attn_masks, attn_vis_pos, attn_vis_neg, attn_vis_self = self.gen_attn(batch)
        ...
        return torch.stack(final_dists, dim=1), attn_dists, coverage, Ias,sim_loss, all_attention_scores, ot_dist, loss_weight
    ...
    def get_attn_diff(self, config, all_attention_scores, attn_pos, attn_neg, attn_vis_self):
        ...
        attn_vis_self = avg_attn_scores * attn_vis_self
        return diff_attn, attn_vis_self
