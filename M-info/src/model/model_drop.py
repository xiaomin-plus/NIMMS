...
from .layer_drop import BertLayer, BertLayerDrop, BertPooler
...
class UniterEncoder(nn.Module):
    def __init__(self, config):
        ...
        layer_drop = BertLayerDrop(config)
        module_list=  [copy.deepcopy(layer)
                                    for _ in range(config.num_hidden_layers-1)]
        module_list.insert(int(config.num_hidden_layers/2),copy.deepcopy(layer_drop))
        self.layer = nn.ModuleList(module_list)

    def forward(self, input_, attention_mask, attn_vis_cross, attn_txt_all,
        ...
        for layer_module in self.layer:
            hidden_states, attention_scores, visual_output = layer_module(hidden_states, attention_mask, attn_vis_cross, attn_txt_all)
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers, all_attention_scores,visual_outputs
...
class UniterModel(UniterPreTrainedModel):
    ...
    def forward(self, input_ids, position_ids,
                img_feat, img_pos_feat,
                attention_mask, attn_vis_cross, attn_txt_all, gather_index=None, img_masks=None,
                output_all_encoded_layers=True,
                txt_type_ids=None, img_type_ids=None):
        ...
        encoded_layers, all_attention_scores,visual_outputs = self.encoder(
            embedding_output, extended_attention_mask, attn_vis_cross, attn_txt_all,
            output_all_encoded_layers=output_all_encoded_layers)
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return encoded_layers, embedding_output,all_attention_scores,visual_outputs
